...
// On window load 
window.onload=createPresignedUrl;
...
$('#start-button').click(function () {
    console.log("In #start-button.click");
...
let streamAudioToWebSocket = function (userMediaStream) {
    console.log("In let streamAudioToWebSocket")
...
function createPresignedUrl() {
    console.log("In function createPresignedUrl");
    let endpoint = "transcribestreaming." + region + ".amazonaws.com:8443";
    Http.open("GET", "https://9hyu0ux6f7.execute-api.us-east-1.amazonaws.com/test");
    Http.send();
  
    Http.onreadystatechange = (e) => {
        if (Http.readyState == 4 && Http.status == 200) {
            console.log(Http.responseText)
            some_wss_url = (JSON.parse(Http.responseText)).body
        }
    }
    return some_wss_url
}   